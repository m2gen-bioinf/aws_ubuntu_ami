...
if [ ! -f /usr/local/R-$R_VERSION/lib/R/bin/R ]; then
    ...
    if [ ! -f R-$R_VERSION.tar.gz ]; then
        echo ">>> Getting https://cran.r-project.org/src/base/R_BASE/R-$R_VERSION.tar.gz"
        wget --no-check-certificate https://cran.r-project.org/src/base/$R_BASE/R-$R_VERSION.tar.gz
    fi
    ...
    make
# reg-test fails in 3.4.1    make check
    make info
    make install
